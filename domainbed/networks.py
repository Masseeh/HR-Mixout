...
from domainbed.models.resnet_imagenet import resnet18, resnet50, resnext18, resnext50
...
class ResNet(torch.nn.Module):
    ...
    def __init__(self, input_shape, hparams, network=None, lora_kwargs=None):
        super(ResNet, self).__init__()

        if hparams["resnet18"]:
            if network is None:
                kwargs = {'pretrained': hparams['pretrained'], 'lora_kwargs': lora_kwargs}
                factory = resnet18
                if hparams["rx"]:  
                    factory = resnext18
        ...
        if nc != 3:
            ...
            ) if not lora_kwargs else lora.LoraConv2d(nc, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False, **lora_kwargs)
...
def Featurizer(input_shape, hparams, lora_kwargs=None):
    ...
    if len(input_shape) == 1:
        ...
        return ResNet(input_shape, hparams, lora_kwargs=lora_kwargs)
    else:
