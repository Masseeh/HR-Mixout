...
def test(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    if args.algorithm == "DiWA":
        ...
        for i, model_test in enumerate(np_model_test):
            ...
            ckp = torch.load(ckp, map_location="cpu", weights_only=False)
        ...
        test_env_str = ",".join(map(str, test_envs))
        ckp = Path(args.model_test[0])/"checkpoints"/f"TE{test_env_str}.pth"
        ckp = torch.load(ckp, map_location="cpu", weights_only=False)
        algorithm.load_state_dict(ckp["model_dict"], strict=True)
