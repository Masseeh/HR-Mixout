...
from domainbed.models import vision_transformer

import domainbed.models.loralib as lora
from domainbed.models.resnet_imagenet import MixConv2d
from domainbed.mixout.mixlinear import MixLinear

@torch.no_grad()
def sma_avg(ema_param, current_param, num_averaged):
    return (ema_param * num_averaged + current_param)/(1.0 + num_averaged)
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.mc_sample = hparams["mc_sample"]

        if hparams["lora_rank"] > 0:
            lora_kwargs = {"r": hparams["lora_rank"], "lora_alpha": 1.0}
        else:
            lora_kwargs = None
            print(f"NOT Using LoRA")

        if lora_kwargs:
            print(f"Using LoRA with rank {lora_kwargs['r']} and alpha {lora_kwargs['lora_alpha']}")

        ## VIT
        if hparams['vit']:
            self.featurizer = vision_transformer.deit_small_patch16_224(pretrained=True, num_classes=0,
                                                                         moe_layers=['M'] * 12, mlp_ratio=4., drop_path_rate=0.0, lora_kwargs=lora_kwargs)
            self.classifier = nn.Sequential(nn.Dropout(self.hparams["resnet_dropout"]), nn.Linear(self.featurizer.num_features, num_classes))
        ## VIT
        else:
            ## ResNet
            self.featurizer = networks.Featurizer(input_shape, self.hparams, lora_kwargs=lora_kwargs)
        ...
        self.drop_zero = hparams["drop_zero"]
        ...
        for mod in self.featurizer.modules():
            if isinstance(mod, (MixConv2d, MixLinear)):
                if self.drop_zero:
        ...
        if self.warmup >= 0:
            self.dropout_reg(r_dp=0.0, g_dp=0.0, fix_mask=False)
        ...
        if hparams["lora_rank"] != 0:
            lora.mark_only_lora_as_trainable(self.featurizer)

        self.wise_alpha = hparams["wise_alpha"]
        if self.wise_alpha > 0.0:
            self.hparams["lora_rank"] = 0
            print(f"Using WiSE with alpha {self.wise_alpha}")
            self.pretrained = networks.Featurizer(input_shape, self.hparams)
            self.freeze_params(self.pretrained, freeze=True)
    ...
    def dropout_reg(self, r_dp, g_dp, fix_mask=False, verbose=True):
        if fix_mask:
            if verbose:print("Fixing Mask")

        if hasattr(self.featurizer, 'dropout'):
        ...
        for mode_name, mod in self.featurizer.named_modules():
            if isinstance(mod, (MixConv2d, MixLinear)):
                ...
                if fix_mask:
                    mod.make_noise(generator=self.generator)
        
        if verbose: print(f"Changed to Dropout: {r_dp}, Group Dropout: {g_dp}")
    ...
    def update(self, x, y, step, **kwargs):
        ...
        if step == self.warmup and (self.r_dp > 0.0 or self.g_dp > 0.0):
            self.dropout_reg(r_dp=self.r_dp, g_dp=self.g_dp, fix_mask=self.hparams["fix_mask"])


        self.optimizer.zero_grad(set_to_none=True)
        
        with autocast(device_type='cuda', enabled=self.hparams["use_amp"]):
    ...
    def post_load(self, **kwargs):
        ...
        if self.mc_sample > 0:
            print(f"MC SAMPLE ON: {self.mc_sample}")

        if self.wise_alpha > 0.0:
            for (name, param), (name_pre, param_pre) in zip(self.featurizer.named_parameters(), self.pretrained.named_parameters()):
                assert name == name_pre
                param.data =  (param_pre.data * self.wise_alpha + param.data * (1 - self.wise_alpha))
    ...
    def predict(self, x):
        ...
        # return out / self.mc_sample
        out = []
        for _ in range(self.mc_sample):

            self.dropout_reg(r_dp=self.r_dp, g_dp=self.g_dp, fix_mask=True, verbose=False)
            out.append(F.log_softmax(self.network(x), dim=-1))
