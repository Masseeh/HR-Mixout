...
def make_block(dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                    drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, cur_depth=0,
                    moe_layers=None, lora_kwargs=None):
    return Block(dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop,
            attn_drop=attn_drop, drop_path=drop_path, norm_layer=norm_layer, act_layer=act_layer,
            cur_depth=cur_depth, moe_layers=moe_layers, lora_kwargs=lora_kwargs)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', moe_layers=None, lora_kwargs=None):
        """
        Args:
        ...
        assert len(moe_layers) == depth if moe_layers is not None else True, f"moe_layers={len(moe_layers)} must have the same length as depth={depth}"
        ...
        self.blocks = nn.ModuleList([
            make_block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,
                cur_depth=i, moe_layers=moe_layers, lora_kwargs=lora_kwargs)
    ...
    def forward_features(self, x, domain_index=None):
        ...
        if self.dist_token is None:
            if isinstance(x, list):
                return [self.pre_logits(xx[:, 0]) for xx in x]
            else:
