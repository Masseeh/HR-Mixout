...
from .vit_layers import MixoutMlp as mixout_mlp
    ...
    for i, block in enumerate(model.blocks.children()):
        if hasattr(block, 'experts'):
            ...
            for e in range(num_experts):
                tmp_keys = block.experts[e].state_dict().keys()
                for key in tmp_keys:
                    state_dict[f'blocks.{i}.experts.{e}.{key}'] = state_dict[f'blocks.{i}.{key}']
                    removed_block.add(f'blocks.{i}.{key}')
