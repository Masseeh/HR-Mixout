...
def accuracy_from_loader(algorithm, loader, weights, debug=False):
    ...
    for i, (x, y) in enumerate(loader):
        ...
        with torch.no_grad(), autocast(device_type='cuda', enabled=algorithm.hparams["use_amp"]):
            logits = algorithm.predict(x)
            if len(logits.shape) > 2:
                ##
                ohot = logits.argmax(dim=-1).permute(1, 0)
                diff = ohot.unsqueeze(0) != ohot.unsqueeze(1)
                pairwise = diff.float().mean(-1).mean()
                overall = (ohot.ne(ohot[0]).any(0)).float().mean()   # scalar
                cr = logits.mean(1).argmax(dim=-1).eq(y).float().mean().item()
                dr = overall
                drate = dr * cr
                disagreement_rate += drate
                ##
